  ...
  if (ust){
    axios.post('http://81.70.183.201:5000/TraditionalPred', {
      ...
      if (ret.code){ // 错误
        ...
        var ses_acc = ret.acc;
        localStorage.setItem("ses_acc", ses_acc);
        {
          document.getElementById("SESACC").innerHTML = 'SES模型的准确率为：' + ses_acc;
        }
...
onMounted(() => {
  const ses_acc = localStorage.getItem("ses_acc");
  if (ses_acc){
    document.getElementById("SESACC").innerHTML = 'SES模型的准确率为：' + ses_acc;
  }
});
        ...
        <Accordion :activeIndex="0">
          <AccordionTab header="Introduction">
            <p class="line-height-3 m-0">
              简单指数平滑（SES）模型是一种用于时间序列预测的简单方法，适用于没有明显趋势和季节性的数据。SES模型基于指数加权移动平均的思想，将较近时刻的观测值赋予较大的权重，以便更好地预测未来时刻的值。它是一种适用于数据波动较小或没有明显变化趋势的情况下的基本预测方法。<br>
              ES模型的核心特点包括：<br>
                指数加权平滑： SES模型使用指数加权平均来平滑时间序列数据。较近的观测值具有较大的权重，较早的观测值具有较小的权重，从而更好地反映近期变化。<br>
                单一参数： SES模型只有一个平滑参数（平滑系数），用于控制历史观测值对预测的影响程度。这个参数通常由用户根据数据特点进行选择。<br>
                适用性： SES模型适用于数据没有明显趋势和季节性的情况，例如稳定的销售数量或价格数据。
            </p>
          </AccordionTab>
    ...
    <div class="col-12">
      <div class="card">
        <Panel header="日准确率" :toggleable="true">
          <p class="line-height-3 m-0" id="SESACC">
