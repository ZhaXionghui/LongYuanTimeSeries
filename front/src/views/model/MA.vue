  ...
  if (ust){
    axios.post('http://81.70.183.201:5000/TraditionalPred', {
      ...
      if (ret.code){ // 错误
        ...
        if(localStorage.getItem("ma_q")){
          localStorage.removeItem("ma_q");
        }
        localStorage.setItem("ma_q", ma_q);
        var ma_acc = ret.acc;
        console.log(ma_acc);
        localStorage.setItem("ma_acc", ma_acc);
        {
          document.getElementById("MAACC").innerHTML = 'AR模型的准确率为：' + ma_acc;}
        {
          var myChart = echarts.init(chart1.value);
          var option = {
            title: {
...
onMounted(() => {
  const ma_acc = localStorage.getItem("ma_acc");
  if (ma_acc){
    document.getElementById("MAACC").innerHTML = 'MA模型的准确率为：' + ma_acc;
  }
});
        ...
        <Accordion :activeIndex="0">
          <AccordionTab header="Introduction">
            <p class="line-height-3 m-0">
              移动平均（MA）模型是一种用于分析和预测时间序列数据的统计方法，它与自回归（AR）模型一起构成了时间序列分析中的两个基本组成部分。<br>
              MA模型基于时间序列数据中过去时刻的白噪声误差项（即随机波动），而不是过去的观测值。这使得MA模型能够捕捉数据中的短期波动和随机性。
              在一个移动平均（MA(q)）模型中，q 代表模型的阶数，表示考虑的过去时刻的白噪声误差项的数量。每个过去时刻的误差项都与一个权重系数相关联，这些权重系数是模型的参数。<br>
              通过调整这些参数，MA模型可以对数据中的随机波动进行建模，并用于未来时刻的预测
            </p>
          </AccordionTab>
    ...
    <div class="col-12">
      <div class="card">
        <Panel header="日准确率" :toggleable="true">
          <p class="line-height-3 m-0" id="MAACC">
