...
const LSTMPredict = () => {
  ...
  if (ust){
    axios.post('http://81.70.183.201:5000/predict', {
      usn,
      ust,
      uet
    }).then(res => {
                ...
                <Accordion :activeIndex="0">
                    <AccordionTab header="Introduction">
                        <p class="line-height-3 m-0">
                            Conv1D层:该层用于提取输入数据的局部特征,对输入数据进行一维卷积操作,并通过指定的参数对卷积进行配置。<br>
                            ReLU激活层:引入非线性特性,增强网络的表达能力。<br>
                            BatchNorm1D层:对隐藏层的输出进行批量归一化,提高训练稳定性和收敛速度。<br>
                            LSTM层:用于对经过特征提取的数据进行时序建模(LSTM单元的架构如右图)。<br>
                            Linear层:全连接层控制输出维度<br>
                            Dropout层:用于随机丢弃部分神经元,防止过拟合。
                        </p>
                    </AccordionTab>
