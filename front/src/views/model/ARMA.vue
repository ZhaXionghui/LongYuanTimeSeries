  ...
  if (ust){
    axios.post('http://81.70.183.201:5000/TraditionalPred', {
      ...
      if (ret.code){ // 错误
        ...
        if(localStorage.getItem("arma_p") && localStorage.getItem("arma_q")){
          localStorage.removeItem("arma_p");
          localStorage.removeItem("arma_q");
        }
        localStorage.setItem("arma_p",arma_p);
        localStorage.setItem("arma_q",arma_q);
        var arma_acc = ret.acc;
        console.log(arma_acc);
        localStorage.setItem("arma_acc", arma_acc);
        {
          document.getElementById("ARMAACC").innerHTML = 'ARMA模型的准确率为：' + arma_acc;
        }
        {
          var myChart = echarts.init(chart1.value);
          var option = {
...
onMounted(() => {
  const arma_acc = localStorage.getItem("arma_acc");
  if (arma_acc){
    document.getElementById("ARMAACC").innerHTML = 'ARMA模型的准确率为：' + arma_acc;
  }
});
        ...
        <Accordion :activeIndex="0">
          <AccordionTab header="Introduction">
            <p class="line-height-3 m-0">
              自回归移动平均（ARMA）模型是一种常用于时间序列分析和预测的统计模型，它结合了自回归（AR）模型和移动平均（MA）模型的特点，以更好地捕捉时间序列数据中的趋势和随机波动。ARMA模型适用于具有自相关性和移动平均性质的时间序列数据。<br>
              ARMA模型的表示为：ARMA(p, q)。其中，p 表示自回归阶数（过去时刻的观测值对当前值的影响），q 表示移动平均阶数（过去时刻的白噪声误差项对当前值的影响）
            </p>
          </AccordionTab>
    ...
    <div class="col-12">
      <div class="card">
        <Panel header="日准确率" :toggleable="true">
          <p class="line-height-3 m-0" id="ARMAACC">
