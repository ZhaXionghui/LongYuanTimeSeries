  ...
  if (ust){
    axios.post('http://81.70.183.201:5000/TraditionalPred', {
      ...
      if (ret.code){ // 错误
        ...
        if(localStorage.getItem("sarima_order")){
          localStorage.removeItem("sarima_order");
          localStorage.removeItem("sarima_seasonal_order");
        }
        localStorage.setItem("sarima_order", sarima_order);
        localStorage.setItem("sarima_seasonal_order", sarima_seasonal_order);

        var sarima_acc = ret.acc;
        localStorage.setItem("sarima_acc", sarima_acc);
        {
          document.getElementById("SARIMAACC").innerHTML = 'SARIMA模型的准确率为：' + sarima_acc;
        }
        {
          var myChart = echarts.init(chart1.value);
          var option = {
...
onMounted(() => {
  const sarima_acc = localStorage.getItem("sarima_acc");
  if (sarima_acc){
    document.getElementById("SARIMAACC").innerHTML = 'SARIMA模型的准确率为：' + sarima_acc;
  }
});
        ...
        <Accordion :activeIndex="0">
          <AccordionTab header="Introduction">
            <p class="line-height-3 m-0">
              季节性自回归移动平均（Seasonal ARIMA，SARIMA）模型是一种用于时间序列分析和预测的扩展模型，是ARIMA模型在处理具有季节性的数据时的一种改进。SARIMA模型结合了自回归（AR）模型、移动平均（MA）模型、差分（I）和季节性成分，以更好地捕捉季节性时间序列数据中的趋势和周期性。<br>  
              SARIMA模型的主要特点包括：<br>
                季节性成分： SARIMA模型引入了季节性成分，用于捕捉数据中的季节性模式。这使得模型可以更好地预测数据在每个季节内的周期性变化。<br>
                差分操作： SARIMA模型在ARIMA的基础上，添加了季节性的差分操作，以处理季节性数据的非平稳性。<br>
                多个参数： 与ARIMA模型不同，SARIMA模型有更多的参数，包括季节性阶数和季节性差分阶数。<br>
                适用性： SARIMA模型适用于具有季节性和趋势性的时间序列数据，例如销售数据、气温数据等。
    ...
    <div class="col-12">
      <div class="card">
        <Panel header="日准确率" :toggleable="true">
          <p class="line-height-3 m-0" id="SARIMAACC">
