  ...
  if (ust){
    axios.post('http://81.70.183.201:5000/TraditionalPred', {
      ...
      if (ret.code){ // 错误
        ...
        var des_acc = ret.acc;
        localStorage.setItem("des_acc", des_acc);
        {
          document.getElementById("DESACC").innerHTML = 'DES模型的准确率为：' + des_acc;
        }
        {
          var myChart = echarts.init(chart1.value);
          var option = {
...
onMounted(() => {
  const des_acc = localStorage.getItem("des_acc");
  if (des_acc){
    document.getElementById("DESACC").innerHTML = 'DES模型的准确率为：' + des_acc;
  }
});
        ...
        <Accordion :activeIndex="0">
          <AccordionTab header="Introduction">
            <p class="line-height-3 m-0">
              双指数平滑（Double Exponential Smoothing，DES）模型是一种用于时间序列预测的方法，它是简单指数平滑（SES）模型的一种扩展，专门用于处理具有趋势的数据。DES模型不仅考虑了数据的平滑，还考虑了数据的线性趋势，使其适用于具有趋势性的时间序列数据。<br>
              DES模型的核心特点包括：<br>
                平滑处理： 类似于SES模型，DES模型也采用指数加权平均的方法，将较近时刻的观测值赋予较大的权重，以平滑时间序列数据。<br>
                趋势处理： DES模型引入了一个额外的指数加权平均来捕捉数据的线性趋势。这意味着DES模型可以更好地预测具有稳定线性增长或下降趋势的时间序列数据。<br>
                两个平滑参数： 与SES模型不同，DES模型有两个平滑参数，一个用于平滑数据，另一个用于平滑趋势。这两个参数需要根据数据的特点进行选择。<br>
            </p>
          </AccordionTab>
    ...
    <div class="col-12">
      <div class="card">
        <Panel header="日准确率" :toggleable="true">
          <p class="line-height-3 m-0" id="DESACC">
