  ...
  if (ust){
    axios.post('http://81.70.183.201:5000/TraditionalPred', {
      ...
      if (ret.code){ // 错误
        ...
        if(localStorage.getItem("arima_p")){
          localStorage.removeItem("arima_p");
          localStorage.removeItem("arima_q");
          localStorage.removeItem("arima_d");
        }
        localStorage.setItem("arima_p",arima_p);
        localStorage.setItem("arima_q",arima_q);
        localStorage.setItem("arima_d",arima_d);

        var arima_acc = ret.acc;
        localStorage.setItem("arima_acc", arima_acc);
        {
          document.getElementById("ARIMAACC").innerHTML = 'ARIMA模型的准确率为：' + arima_acc;
        }
        {
          var myChart = echarts.init(chart1.value);
          var option = {
...
onMounted(() => {
  const arima_acc = localStorage.getItem("arima_acc");
  if (arima_acc){
    document.getElementById("ARIMAACC").innerHTML = 'ARIMA模型的准确率为：' + arima_acc;
  }
});
        ...
        <Accordion :activeIndex="0">
          <AccordionTab header="Introduction">
            <p class="line-height-3 m-0">
              模型简介差分积分移动平均自回归（ARIMA）模型是一种用于时间序列分析和预测的统计方法，用于处理具有趋势和季节性的非平稳时间序列数据。ARIMA模型结合了自回归（AR）模型、移动平均（MA）模型以及差分（I）的概念，可以用来平稳化非平稳数据，捕捉长期趋势和短期波动，并进行未来时刻的预测。<br>
              ARIMA模型包括三种成分：<br>
                自回归（AR）成分： ARIMA模型可以捕捉过去时刻的观测值对当前值的影响，用于捕捉数据的长期趋势。<br>
                移动平均（MA）成分： ARIMA模型可以考虑过去时刻的白噪声误差项对当前值的影响，用于捕捉数据的短期波动。<br>
                差分（I）成分： 差分是对时间序列数据进行减法操作，用于将非平稳数据转化为平稳数据。差分操作可以帮助消除趋势和季节性，使得数据更容易建模。
            </p>
          </AccordionTab>
    ...
    <div class="col-12">
      <div class="card">
        <Panel header="日准确率" :toggleable="true">
          <p class="line-height-3 m-0" id="ARIMAACC">
