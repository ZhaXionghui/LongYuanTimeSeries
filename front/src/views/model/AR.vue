  ...
  if (ust){
    ...
    axios.post('http://81.70.183.201:5000/TraditionalPred', {
      ...
      if (ret.code){ // 错误
        ...
        localStorage.setItem("ar_p", ar_p);

        var ar_acc = ret.acc;
        localStorage.setItem("ar_acc", ar_acc);
        {
          document.getElementById("ARACC").innerHTML = 'AR模型的准确率为：' + ar_acc;
        }
        {
          var myChart = echarts.init(chart1.value);
          var option = {
...
onMounted(() => {
  const ar_acc = localStorage.getItem("ar_acc");
  if (ar_acc){
    document.getElementById("ARACC").innerHTML = 'AR模型的准确率为：' + ar_acc;
  }
});
        ...
        <Accordion :activeIndex="0">
          <AccordionTab header="Introduction">
            <p class="line-height-3 m-0">
              时间序列自回归（AR）模型是一种用于分析和预测时间序列数据的统计方法。<br>
              它基于时间序列数据中过去时刻的观测值，建立一个线性模型来预测未来时刻的值。AR模型适用于具有自相关性（即当前值与过去值相关）的时间序列数据。<br>
              AR模型的核心思想是利用历史时刻的观测值来预测当前时刻或未来时刻的值。
            </p>
          </AccordionTab>
...
<!--          <Button label="开始预测" class="mr-2 mb-2" @click="LSTMPredict" />-->
    ...
    <div class="card">
      <h5>日准确率</h5>
      <Panel>
        <p class="line-height-3 m-0" id="ARACC">
